...
df = pd.DataFrame(columns=['Day', 'Stock Price', 'Portfolio Value', 'Cash Deposited',
                  'Basis with Harvesting', 'Basis without Harvesting'])
...
for day in range(1, days):
    ...
    df.loc[day] = [day, stock_price, portfolio_value, initial_cash,
                   basis, shares * initial_stock_price, cumulative_loss]
...
def init():
    ...
    ax.legend(handles=legend_labels, labels=['Portfolio Value', 'Basis with Harvesting', 'Basis without Harvesting', 'Harvested Loss',
              'Cumulative Harvested Losses'], loc='lower left', bbox_to_anchor=(0, 0.05), ncol=1, frameon=False, prop=orpheus_font)
    ...
    ax.annotate('Portfolio Basis After Harvesting Losses', xy=(0.5, 1), xycoords='axes fraction', fontsize=32,
...
def update(day):
    ...
    if day > 0 and df['Basis with Harvesting'][day] < df['Basis with Harvesting'][day - 1]:
        ...
        ax.scatter(day, df['Portfolio Value'][day], s=normalized_loss * 10000,
                   color='green', label='Harvested Loss' if day == 1 else "")
    ...
    # Label for final data point of cumulative harvested losses
    if day == days - 1:
